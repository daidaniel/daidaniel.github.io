...
Constants outside `CONFIG` (edit in place if needed): physics step clamp 50 ms with 4 substeps, danger smoothing 10/s, planet-velocity smoothing 2/s with a 0.08 energy-danger deadband, trail 40 points sampled every 25 ms, joystick radius 0.07 ×u, asteroid cull margin 0.12 ×u (culling is off in the tutorial), tutorial gate dwell 2.5 s, timed captions 2.8–3.5 s, shake-step timeout 25 s, tutorial asteroid distance 0.55 ×u and speed 1.5× `asteroids.speed[0]`, starfield ~1 star per 15,000 px², thrust puffs ~20/s, fade 300 ms out / 400 ms in, burst-dump threshold 0.15 ×u per frame.
...
- Art: four runtime-generated textures, no asset files — `disc` and jittered-polygon `rock` (white, tinted per object), `planet` (canvas: amber-500 shadow disc with an amber-300 lit circle offset 18 px toward the top-left — a crescent-shaded ball, shadow on the bottom-right), and `stars` (the whole starfield baked into one viewport-sized canvas: 1–3 px zinc/white dots, ~1 per 15,000 px², alpha 0.4–0.8, static, one draw call, rebuilt on resize). Thrust shows as wide faint amber puffs — radius ≈ 0.57× planet radius, alpha 0.12, ~20 particles/s, spawn jittered ±0.4× planet radius perpendicular to thrust. Swap textures for SVGs by loading real ones and deleting `makeTextures()`.
