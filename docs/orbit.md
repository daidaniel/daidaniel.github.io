...
- Art: four runtime-generated textures, no asset files — `disc` and jittered-polygon `rock` (white, tinted per object), `planet` (canvas: amber-300 disc, three wavy amber-500 latitude bands clipped by `source-atop`, random waviness per load), and `glow` (radial-gradient falloff). Thrust shows as a soft amber glow on the planet's trailing edge (alpha eases to 0.35 at 10/s) plus amber wisps throttled to ~30 particles/s. A starfield of 1–3 px zinc/white dots (~1 per 15,000 px², base alpha 0.4–0.8, twinkling to 0.6× base on 2–6 s cycles — never blinking out — static under `prefers-reduced-motion`) sits at depth 0 and rebuilds on resize. Swap textures for SVGs by loading real ones and deleting `makeTextures()`.
