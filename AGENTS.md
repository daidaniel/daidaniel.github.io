...
The entire site is `src/layouts/Base.astro` (head, self-hosted fonts, OG tags, build-time footer year) wrapping two pages — `src/pages/index.astro` and `404.astro` — plus the single shared `TextLink.astro`. The non-obvious, cross-file details:
